...
function defaultLive(c) {
  ...
  return {
    companyId: c.id, companyName: c.name, price, open,
    high: (c.dayHigh  > 0) ? c.dayHigh  : price,
    low:  (c.dayLow   > 0) ? c.dayLow   : price,
    dayChangePct: 0, tickChangePct: 0, history: [price],
  };
}
...
function InteractiveChart({ history = [], open, high, low, width = 700, height = 180 }) {
  ...
  if (history.length < 2) return (
    <div style={{ width, height, display:"flex", alignItems:"center", justifyContent:"center", color:"#4a5a7a", fontSize:11, fontFamily:"'DM Mono',monospace", letterSpacing:2 }}>AWAITING DATA…</div>
  );
  const PAD_L=58, PAD_R=14, PAD_T=14, PAD_B=28;
  const W=width-PAD_L-PAD_R, H=height-PAD_T-PAD_B;
  const pMin=Math.min(...history)*0.9985, pMax=Math.max(...history)*1.0015, pRng=pMax-pMin||1;
  const px=(i)=>PAD_L+(i/(history.length-1))*W;
  const py=(v)=>PAD_T+H-((v-pMin)/pRng)*H;
  const linePts=history.map((v,i)=>`${px(i)},${py(v)}`).join(" ");
  const areaPath=`M${px(0)},${py(history[0])} `+history.map((v,i)=>`L${px(i)},${py(v)}`).join(" ")+` L${px(history.length-1)},${PAD_T+H} L${px(0)},${PAD_T+H} Z`;
  const isUp=history[history.length-1]>=history[0], color=isUp?teal:red;
  const yTicks=Array.from({length:5},(_,i)=>({v:pMin+(pRng*i)/4,y:py(pMin+(pRng*i)/4)}));
  ...
  const handleMove=useCallback((e)=>{
    ...
    const i=Math.max(0,Math.min(history.length-1,Math.round(((mx-PAD_L)/W)*(history.length-1))));
    setHover({x:px(i),y:py(history[i]),idx:i,price:history[i],pct:((history[i]-history[0])/history[0])*100});
  },[history]);
  return (
    <div style={{position:"relative",userSelect:"none"}}>
      <svg ref={svgRef} width={width} height={height} viewBox={`0 0 ${width} ${height}`} style={{display:"block",cursor:"crosshair"}} onMouseMove={handleMove} onMouseLeave={()=>setHover(null)}>
        <defs>
          <linearGradient id={`${uid}f`} x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor={color} stopOpacity="0.18"/><stop offset="100%" stopColor={color} stopOpacity="0"/></linearGradient>
          <linearGradient id={`${uid}l`} x1="0" x2="1" y1="0" y2="0"><stop offset="0%" stopColor={color} stopOpacity="0.35"/><stop offset="100%" stopColor={color} stopOpacity="1"/></linearGradient>
          <clipPath id={`${uid}c`}><rect x={PAD_L} y={PAD_T} width={W} height={H}/></clipPath>
        </defs>
        {yTicks.map((t,i)=><line key={i} x1={PAD_L} y1={t.y} x2={PAD_L+W} y2={t.y} stroke="rgba(255,255,255,0.04)" strokeWidth="1"/>)}
        ...
        {yTicks.map((t,i)=><text key={i} x={PAD_L-6} y={t.y+4} textAnchor="end" fontSize="8.5" fill="rgba(255,255,255,0.2)" fontFamily="'DM Mono',monospace">{fmt(t.v,0)}</text>)}
        {openY&&<text x={PAD_L+W+3} y={openY+3.5} fontSize="8" fill={amber} fontFamily="'DM Mono',monospace" opacity="0.7">OPEN</text>}
        {xTicks.map(i=><text key={i} x={px(i)} y={PAD_T+H+18} textAnchor="middle" fontSize="8.5" fill="rgba(255,255,255,0.15)" fontFamily="'DM Mono',monospace">T-{history.length-1-i}s</text>)}
        {!hover&&<circle cx={px(history.length-1)} cy={py(history[history.length-1])} r="3.5" fill={color} style={{filter:`drop-shadow(0 0 4px ${color})`}}/>}
        {hover&&<>
      ...
      {hover&&(()=>{
        const hUp=hover.pct>=0,c=hUp?teal:red,flip=hover.x>width*0.65;
        return <div style={{position:"absolute",top:Math.max(4,hover.y-52),left:flip?hover.x-148:hover.x+10,background:"#0a1525",border:`1px solid ${c}55`,borderRadius:8,padding:"9px 14px",pointerEvents:"none",zIndex:10,minWidth:130,boxShadow:"0 4px 24px rgba(0,0,0,0.6)"}}>
...
function CandleChart({ candles=[], width=700, height=160 }) {
  const [hover,setHover]=useState(null);
  if(!candles.length) return <div style={{width,height,display:"flex",alignItems:"center",justifyContent:"center",color:"#4a5a7a",fontSize:11,fontFamily:"'DM Mono',monospace",letterSpacing:2}}>NO CANDLE DATA</div>;
  const PAD_L=10,PAD_R=10,PAD_T=10,PAD_B=24,W=width-PAD_L-PAD_R,H=height-PAD_T-PAD_B;
  const all=candles.flatMap(c=>[c.open,c.close,c.high,c.low]);
  const pMax=Math.max(...all)*1.002,pMin=Math.min(...all)*0.998,pRng=pMax-pMin||1;
  const py=(v)=>PAD_T+H-((v-pMin)/pRng)*H;
  const barW=Math.max(2,Math.floor((W/candles.length)*0.7)),gap=W/candles.length;
  return (
    <div style={{position:"relative",userSelect:"none"}}>
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} style={{display:"block"}} onMouseLeave={()=>setHover(null)}>
        {candles.map((c,i)=>{
          const cx=PAD_L+i*gap+gap/2,isUp=c.close>=c.open,col=isUp?teal:red;
          const bodyTop=py(Math.max(c.open,c.close)),bodyH=Math.max(1,py(Math.min(c.open,c.close))-bodyTop);
          return <g key={i} onMouseEnter={()=>setHover({c,cx})} style={{cursor:"crosshair"}}>
      ...
      {hover&&(()=>{
        const isUp=hover.c.close>=hover.c.open,c=isUp?teal:red,flip=hover.cx>width*0.6;
        return <div style={{position:"absolute",top:8,left:flip?hover.cx-180:hover.cx+10,background:"#0a1525",border:`1px solid ${c}55`,borderRadius:8,padding:"10px 14px",pointerEvents:"none",zIndex:10,minWidth:160,boxShadow:"0 4px 20px rgba(0,0,0,0.6)"}}>
      ...
      <table style={{width:"100%",borderCollapse:"collapse"}}>
        <thead>
          <tr style={{borderBottom:"1px solid #0e1828",position:"sticky",top:0,background:"#060d1a"}}>
            {["Time","Price","Δ"].map(h=><th key={h} style={{padding:"7px 12px",fontSize:9,color:"#5a6a8a",letterSpacing:2,textTransform:"uppercase",fontFamily:"'DM Mono',monospace",textAlign:h!=="Time"?"right":"left"}}>{h}</th>)}
          </tr>
        </thead>
        <tbody>
          {[...rows].reverse().slice(0,20).map((row,i,arr)=>{
            const prev=arr[i+1],delta=prev?row.price-prev.price:0,up=delta>=0;
            return <tr key={row.id||i} style={{borderBottom:"1px solid rgba(255,255,255,0.02)"}}
...
// ─────────────────────────────────────────────────────────────────────────────
// ORDER PANEL — FIXED: uses tradeCompanyId (trade-service ID) not company.id
// ─────────────────────────────────────────────────────────────────────────────
function OrderPanel({ company, livePrice, tradeCompanyId }) {
  // KEY FIX: trade-service and stock-service have separate DBs with different
  // auto-increment IDs. Always use tradeCompanyId when posting to 8083.
  ...
  const submit = async () => {
    ...
    if (!effectiveCompanyId) {
      setStatus({ ok:false, msg:"Company not found in trade-service. Sync may still be running." });
      return;
    }
    ...
    try {
      if (pfMode && portfolioId) {
        ...
        // Build body — only include price fields when needed
        const body = {
          userId:    userId,
          companyId: effectiveCompanyId,   // ← trade-service ID
          quantity:  parseInt(qty),
          orderType: orderType,
        };
        if ((orderType === "LIMIT" || orderType === "GTT") && limitPx) body.price = parseFloat(limitPx);
        if (orderType === "GTT" && triggerPx) body.triggerPrice = parseFloat(triggerPx);

        console.log("[OrderPanel] Sending to 8083:", body);
      ...
      const msg = e.response?.data?.message || e.response?.data?.error || JSON.stringify(e.response?.data) || "Order failed";
      console.error("[OrderPanel] Error:", e.response?.data);
    ...
    <div>
      ...
      {pfMode && (
        <div style={{ marginBottom:12 }}>
          ...
          {portfolios.length === 0 && (
            <div style={{ fontSize:9, color:amber, fontFamily:"'DM Mono',monospace", marginTop:5 }}>⚠ No portfolios found — create one in the Portfolio tab first</div>
          )}
        </div>
      )}
      ...
      {[
        { label:"Quantity", val:qty, set:setQty, show:true, ph:"1" },
        { label:"Price Per Share (₹)", val:limitPx, set:setLimitPx, show:pfMode, ph:`₹${fmt(livePrice,0)}` },
        { label:"Limit Price (₹)", val:limitPx, set:setLimitPx, show:!pfMode&&(orderType==="LIMIT"||orderType==="GTT"), ph:`e.g. ${fmt(livePrice,0)}` },
        { label:"Trigger Price (₹)", val:triggerPx, set:setTriggerPx, show:!pfMode&&orderType==="GTT", ph:"Trigger at…" },
...
function ChartInsights({ history=[], open, high, low, price, dayChangePct }) {
  ...
  const ups=changes.filter(c=>c>0).length,downs=changes.filter(c=>c<0).length;
...
function LiveSparkline({ history=[], width=80, height=32 }) {
  ...
  const max=Math.max(...history),min=Math.min(...history),up=history[history.length-1]>=history[0],c=up?teal:red;
...
// ─────────────────────────────────────────────────────────────────────────────
// LIVE COMPANY ROW  — unchanged from doc 33
// ─────────────────────────────────────────────────────────────────────────────
function LiveCompanyRow({ company, liveData, onClick, watchlist, alerts, onToggleWatch, onSetAlert }) {
  ...
  const flash=useFlash(price),up=dayChangePct>=0,color=up?teal:red;
      ...
      <td style={{padding:"12px 10px"}}>
        <div style={{display:"flex",gap:5,alignItems:"center"}}>
          <button title={isWatched?"Remove from watchlist":"Add to watchlist"}
            ...
            style={{width:28,height:28,borderRadius:7,cursor:"pointer",border:`1px solid ${isWatched?"rgba(251,191,36,0.55)":"rgba(255,255,255,0.1)"}`,background:isWatched?"rgba(251,191,36,0.14)":"rgba(255,255,255,0.03)",color:isWatched?amber:"#3a4a6a",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.15s"}}
          ...
          <button title={hasAlert?"Edit price alert":"Set price alert"}
            onClick={e=>{e.stopPropagation();onSetAlert&&onSetAlert(company);}}
            style={{width:28,height:28,borderRadius:7,cursor:"pointer",border:`1px solid ${hasAlert?"rgba(0,184,255,0.55)":"rgba(255,255,255,0.1)"}`,background:hasAlert?"rgba(0,184,255,0.14)":"rgba(255,255,255,0.03)",color:hasAlert?blue:"#3a4a6a",fontSize:13,display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.15s"}}
...
// ─────────────────────────────────────────────────────────────────────────────
// LIVE DETAIL MODAL — KEY FIX: passes liveData.companyId to OrderPanel
// ─────────────────────────────────────────────────────────────────────────────
function LiveDetailModal({ company, liveData, onClose }) {
  const { price,open,high,low,dayChangePct,tickChangePct,history }=liveData;
  const up=dayChangePct>=0,color=up?teal:red,flash=useFlash(price);
  ...
  const views=[{id:"live",label:"Live Chart"},{id:"candle",label:"Candles"},{id:"history",label:"History"},{id:"trade",label:"Trade"}];
  return (
    <div onClick={e=>{if(e.target===e.currentTarget)onClose();}}
      ...
      <div style={{background:"#080f1e",border:`1px solid ${color}33`,borderTop:`2px solid ${color}`,borderRadius:20,width:"100%",maxWidth:920,maxHeight:"96vh",overflowY:"auto",position:"relative"}}>
        <button onClick={onClose} style={{position:"absolute",top:16,right:18,background:"rgba(255,255,255,0.04)",border:"1px solid #192030",color:"#4a5a7a",width:32,height:32,borderRadius:8,cursor:"pointer",fontSize:15,display:"flex",alignItems:"center",justifyContent:"center",zIndex:2}}
              ...
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:5}}>
                ...
                {hasLiveData?(
                  <span style={{background:"rgba(0,212,160,0.1)",border:"1px solid rgba(0,212,160,0.3)",color:teal,fontSize:9,padding:"4px 10px",borderRadius:4,fontFamily:"'DM Mono',monospace",letterSpacing:2,display:"flex",alignItems:"center",gap:5}}>
                    <span style={{width:5,height:5,borderRadius:"50%",background:teal,display:"inline-block",animation:"pulseDot 1.5s infinite"}}/>WS LIVE · 1S
                  </span>
                ):(
                  <span style={{background:"rgba(255,90,106,0.1)",border:"1px solid rgba(255,90,106,0.3)",color:red,fontSize:9,padding:"4px 10px",borderRadius:4,fontFamily:"'DM Mono',monospace",letterSpacing:2}}>NOT IN TRADE-SERVICE</span>
                )}
                <CircuitBadge dayPct={dayChangePct}/>
              </div>
              <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:14,fontWeight:600,color:"#6a7a9a"}}>{company.name} · {company.sector} · ID #{company.id}</div>
            </div>
            <div style={{textAlign:"right"}}>
              {hasLiveData?(
                <>
                  <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:36,fontWeight:800,letterSpacing:-1,lineHeight:1,color:flash?(flash==="up"?teal:red):"#eef0f8",transition:"color 0.4s"}}>₹{fmt(price)}</div>
                  <div style={{marginTop:5,display:"flex",justifyContent:"flex-end",gap:8}}>
                    <span style={{background:`${color}18`,border:`1px solid ${color}33`,color,padding:"4px 12px",borderRadius:20,fontSize:12,fontFamily:"'DM Mono',monospace",fontWeight:600}}>{up?"+":""}{dayChangePct.toFixed(2)}% day</span>
                    <span style={{background:"rgba(255,255,255,0.04)",border:"1px solid #192030",color:"#3a4a6a",padding:"4px 10px",borderRadius:20,fontSize:11,fontFamily:"'DM Mono',monospace"}}>{tickChangePct>=0?"+":""}{tickChangePct.toFixed(2)}% tick</span>
                  </div>
                </>
              ):(
                <div style={{color:"#2a3550",fontFamily:"'DM Mono',monospace",fontSize:12,marginTop:8}}>Add to trade-service<br/>to enable live pricing</div>
              )}
            </div>
          </div>
        ...
        <div style={{display:"flex",borderBottom:"1px solid #0e1828"}}>
          {views.map(v=><button key={v.id} onClick={()=>setActiveView(v.id)}
        ...
        {activeView==="live"&&(!hasLiveData?(
          <div style={{padding:"48px 28px",textAlign:"center"}}>
            ...
            <div style={{color:"#2a3550",fontSize:11}}>Add this company to trade-service (port 8083) with an openingPrice.</div>
          </div>
        ):(
          <>
            ...
            <div style={{padding:"14px 28px 0"}}>
              <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:2}}>
                {[{label:"Open",val:`₹${fmt(open)}`},{label:"Live Price",val:`₹${fmt(price)}`,hi:tickChangePct>0,lo:tickChangePct<0},{label:"Day High",val:`₹${fmt(high)}`,hi:true},{label:"Day Low",val:`₹${fmt(low)}`,lo:true},{label:"Day Change",val:`${up?"+":""}${dayChangePct.toFixed(2)}%`,hi:up,lo:!up},{label:"Tick Change",val:`${tickChangePct>=0?"+":""}${tickChangePct.toFixed(2)}%`,hi:tickChangePct>0,lo:tickChangePct<0},{label:"Tick Limit",val:"±5% / tick"},{label:"Circuit",val:"±20% / day"},{label:"Data Points",val:history.length},{label:"Interval",val:"1 second"}].map((s,i)=>(
            ...
            {indicators&&(
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
                ...
                <div style={{background:"#060d1a",border:"1px solid #0e1828",borderRadius:12,padding:"18px 20px"}}>
                  ...
                  <div style={{fontSize:11,color:price>=indicators.MovingAverage?teal:red,fontFamily:"'DM Mono',monospace",marginBottom:12}}>{price>=indicators.MovingAverage?"▲ Price above MA — bullish":"▼ Price below MA — bearish"}</div>
                  <div style={{fontSize:9,color:"#5a6a8a",fontFamily:"'DM Mono',monospace"}}>{((price-indicators.MovingAverage)/indicators.MovingAverage*100>=0?"+":"")+((price-indicators.MovingAverage)/indicators.MovingAverage*100).toFixed(3)}% from MA</div>
                </div>
              </div>
            )}
          </div>
        ...
        {/* TRADE — passes liveData.companyId so OrderPanel uses the trade-service ID */}
        {activeView==="trade"&&(
          <div style={{padding:"16px 28px"}}>
            {!hasLiveData?(
              <div style={{padding:"32px",textAlign:"center",color:"#4a5a7a",fontSize:12,fontFamily:"'DM Mono',monospace"}}>Company not in trade-service — cannot place orders</div>
            ):(
...
// ─────────────────────────────────────────────────────────────────────────────
// ALERT MODAL — unchanged from doc 33
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// WATCHLIST CARD — unchanged from doc 33
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// SECTOR DONUT — unchanged from doc 33
// ─────────────────────────────────────────────────────────────────────────────
const SECTOR_COLORS = {
  "IT":"#00b8ff","Banking":"#00d4a0","Energy":"#fbbf24","FMCG":"#a78bfa",
  "Telecom":"#f472b6","Infrastructure":"#fb923c","Auto":"#34d399",
  "Chemicals":"#60a5fa","Consumer Goods":"#f87171","Other":"#6b7280","Finance":"#a78bfa",
};
...
function SectorDonut({ portfolios, liveMap, companies }) {
  ...
  const slices=entries.map(([sector,value])=>{
    const pct=value/total,sweep=pct*2*Math.PI;
      ...
      <div style={{display:"flex",gap:24,alignItems:"center",flexWrap:"wrap"}}>
        <div style={{position:"relative",flexShrink:0}}>
          <svg width={220} height={220} viewBox="0 0 220 220">
            {slices.map(s=>(
              <path key={s.sector} d={s.path} fill={SECTOR_COLORS[s.sector]||SECTOR_COLORS["Other"]}
                opacity={hovered?(hovered===s.sector?1:0.3):0.85}
                style={{cursor:"pointer",transition:"opacity 0.2s"}}
                onMouseEnter={()=>setHovered(s.sector)} onMouseLeave={()=>setHovered(null)}/>
            ))}
            <text x={CX} y={CY-8} textAnchor="middle" fontSize="11" fill={hov?(SECTOR_COLORS[hov.sector]||"#c8d4f0"):"#5a6a8a"} fontFamily="'DM Mono',monospace">{hov?hov.sector:"Total"}</text>
            <text x={CX} y={CY+10} textAnchor="middle" fontSize="14" fill={hov?(SECTOR_COLORS[hov.sector]||"#eef0f8"):"#eef0f8"} fontFamily="'Plus Jakarta Sans',sans-serif" fontWeight="700">{hov?`${(hov.pct*100).toFixed(1)}%`:`₹${fmtCap(total)}`}</text>
            {hov&&<text x={CX} y={CY+26} textAnchor="middle" fontSize="10" fill="#5a6a8a" fontFamily="'DM Mono',monospace">₹{fmtCap(hov.value)}</text>}
          </svg>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:8,flex:1,minWidth:160}}>
          {slices.map(s=>(
            <div key={s.sector} style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer",opacity:hovered&&hovered!==s.sector?0.4:1,transition:"opacity 0.2s"}}
              onMouseEnter={()=>setHovered(s.sector)} onMouseLeave={()=>setHovered(null)}>
...
// ─────────────────────────────────────────────────────────────────────────────
// SIP SIMULATOR — unchanged from doc 33
// ─────────────────────────────────────────────────────────────────────────────
const SIP_HISTORY = {
...
function SipSimulator({ companies, liveMap }) {
  ...
  const monthly_data=slice.map((price,i)=>{
    ...
    totalUnits+=units; totalInvested+=amount;
    ...
    <div className="glass-card" style={{borderRadius:14,padding:"20px 24px"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:18}}>
        <div>
          ...
          <select value={symbol} onChange={e=>setSymbol(e.target.value)} style={{background:"#060d1a",border:"1px solid #0e1828",borderRadius:7,padding:"9px 12px",color:"#eef0f8",fontFamily:"'DM Mono',monospace",fontSize:12,width:"100%",outline:"none",cursor:"pointer"}}>
        ...
        <div>
          ...
          <input type="number" value={monthly} onChange={e=>setMonthly(e.target.value)} min="100" step="500"
            ...
            onFocus={e=>e.target.style.borderColor=`${purple}55`} onBlur={e=>e.target.style.borderColor="#0e1828"}/>
        </div>
        <div>
          ...
          <input type="range" min="3" max="24" value={months} onChange={e=>setMonths(Number(e.target.value))} style={{width:"100%",marginTop:10,accentColor:purple}}/>
      ...
      <div style={{background:"#060d1a",borderRadius:10,padding:"8px 4px 4px",position:"relative"}} onMouseLeave={()=>setHovIdx(null)}>
        <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{display:"block"}}
          onMouseMove={e=>{
            const rect=e.currentTarget.getBoundingClientRect();
            const mx=(e.clientX-rect.left)/rect.width*W;
            if(mx<PAD_L||mx>PAD_L+cW){setHovIdx(null);return;}
            setHovIdx(Math.max(0,Math.min(vals.length-1,Math.round(((mx-PAD_L)/cW)*(vals.length-1)))));
          }}>
          <defs>
            <linearGradient id={`${uid}_g`} x1="0" x2="0" y1="0" y2="1">
              <stop offset="0%" stopColor={up?teal:red} stopOpacity="0.2"/><stop offset="100%" stopColor={up?teal:red} stopOpacity="0"/>
            </linearGradient>
          </defs>
          {[0.25,0.5,0.75,1].map(f=>{
            const yy=PAD_T+cH*(1-f),vv=minV+(maxV-minV)*f;
            return <g key={f}><line x1={PAD_L} y1={yy} x2={PAD_L+cW} y2={yy} stroke="rgba(255,255,255,0.04)" strokeWidth="1"/><text x={PAD_L-6} y={yy+4} textAnchor="end" fontSize="8" fill="rgba(255,255,255,0.2)" fontFamily="'DM Mono',monospace">{fmtCap(vv)}</text></g>;
          })}
          ...
          {slice.map((_,i)=>{
            ...
            return <text key={i} x={px(i)} y={PAD_T+cH+18} textAnchor="middle" fontSize="8" fill="rgba(255,255,255,0.2)" fontFamily="'DM Mono',monospace">{SIP_MONTHS[mIdx]}</text>;
          })}
          {hovIdx!==null&&<>
        ...
        {hovIdx!==null&&monthly_data[hovIdx]&&(()=>{
          const d=monthly_data[hovIdx],ret=d.curVal-d.totalInvested,rp=d.totalInvested>0?(ret/d.totalInvested)*100:0,u=ret>=0;
          return <div style={{position:"absolute",top:8,left:px(hovIdx)>W*0.6?"auto":px(hovIdx)/W*100+2+"%",right:px(hovIdx)>W*0.6?(100-px(hovIdx)/W*100)+2+"%":"auto",background:"#0a1525",border:`1px solid ${u?teal:red}44`,borderRadius:8,padding:"9px 13px",pointerEvents:"none",minWidth:150,zIndex:10}}>
            ...
            {[["Invested",`₹${fmtCap(d.totalInvested)}`,"#c8d4f0"],["Value",`₹${fmtCap(d.curVal)}`,u?teal:red],["Return",`${u?"+":""}${rp.toFixed(2)}%`,u?teal:red],["Units",d.totalUnits.toFixed(3),amber]].map(([l,v,c])=>(
        ...
        <table style={{width:"100%",borderCollapse:"collapse"}}>
          ...
          <tbody>
            {monthly_data.map((d,i)=>{
              const ret=d.curVal-d.totalInvested,rp=d.totalInvested>0?(ret/d.totalInvested)*100:0,u=ret>=0;
              const mIdx=(new Date().getMonth()-slice.length+1+i+120)%12;
              return <tr key={i} style={{borderBottom:"1px solid rgba(255,255,255,0.02)",background:hovIdx===i?"rgba(255,255,255,0.03)":"transparent"}}
...
// ─────────────────────────────────────────────────────────────────────────────
// PORTFOLIO MODAL — unchanged from doc 33
// ─────────────────────────────────────────────────────────────────────────────
function PortfolioModal({ mode, portfolio, onClose, onSaved }) {
  ...
  const submit=async()=>{
    ...
    try {
      if(isEdit) await PORTFOLIO_API.put(`/portfolios/${portfolio.id}`,{name:name.trim(),description:desc.trim()});
      else       await PORTFOLIO_API.post("/portfolios",{name:name.trim(),description:desc.trim()});
      onSaved();onClose();
    } catch(e){setError(e.response?.data?.message||"Failed to save portfolio");}
  ...
  return (
    <div onClick={e=>{if(e.target===e.currentTarget)onClose();}}
      ...
      <div style={{background:"#080f1e",border:`1px solid ${c}33`,borderTop:`2px solid ${c}`,borderRadius:18,width:"100%",maxWidth:480,padding:"28px 32px",position:"relative"}}>
        <button onClick={onClose} style={{position:"absolute",top:14,right:16,background:"rgba(255,255,255,0.04)",border:"1px solid #192030",color:"#4a5a7a",width:30,height:30,borderRadius:7,cursor:"pointer",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center"}}
          onMouseEnter={e=>e.currentTarget.style.color="#eef0f8"} onMouseLeave={e=>e.currentTarget.style.color="#4a5a7a"}>✕</button>
        <div style={{fontSize:9,color:"#5a6a8a",letterSpacing:3,textTransform:"uppercase",fontFamily:"'DM Mono',monospace",marginBottom:6}}>{isEdit?"Edit Portfolio":"New Portfolio"}</div>
        <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:20,fontWeight:800,color:"#eef0f8",marginBottom:22}}>{isEdit?`Rename "${portfolio.name}"`:"Create a portfolio"}</div>
        {error&&<div style={{background:"rgba(255,90,106,0.08)",border:"1px solid rgba(255,90,106,0.3)",borderRadius:8,padding:"10px 14px",marginBottom:14,fontSize:11,color:red,fontFamily:"'DM Mono',monospace"}}>✕ {error}</div>}
        <div style={{marginBottom:14}}>
          ...
          <input value={name} onChange={e=>setName(e.target.value)} placeholder="e.g. Long Term Growth"
        ...
        <div style={{marginBottom:22}}>
          ...
          <textarea value={desc} onChange={e=>setDesc(e.target.value)} placeholder="e.g. Blue-chip stocks for 5+ year horizon" rows={3}
        ...
        <div style={{display:"flex",gap:10}}>
          <button onClick={onClose} style={{flex:1,padding:"11px",background:"transparent",border:"1px solid #0e1828",color:"#3a4a6a",borderRadius:9,cursor:"pointer",fontFamily:"'DM Mono',monospace",fontSize:10,letterSpacing:2}}
          ...
          <button onClick={submit} disabled={saving} style={{flex:2,padding:"11px",background:`${c}18`,border:`1px solid ${c}55`,color:c,borderRadius:9,cursor:saving?"wait":"pointer",fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:700,fontSize:14,opacity:saving?0.7:1,transition:"all 0.15s"}}
...
// ─────────────────────────────────────────────────────────────────────────────
// PORTFOLIO CARD — unchanged from doc 33
// ─────────────────────────────────────────────────────────────────────────────
function PortfolioCard({ portfolio, liveMap, companies, onEdit, onDelete, onRefresh }) {
  ...
  const pnl=currentValue-totalInvested,pnlPct=totalInvested>0?(pnl/totalInvested)*100:0,up=pnl>=0;
  const loadTxns=async()=>{
    setTxLoading(true);
    try{const res=await PORTFOLIO_API.get(`/transactions/portfolio/${portfolio.id}`);setTxns(Array.isArray(res.data)?res.data:[]);}
    catch(e){setTxns([]);}
    finally{setTxLoading(false);}
  };
  const handleTxView=()=>{setTxView(v=>{if(!v)loadTxns();return !v;});};
  return (
    <div style={{borderRadius:16,overflow:"hidden",transition:"all 0.2s",background:"rgba(10,16,32,0.5)",backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)",border:"1px solid rgba(255,255,255,0.07)"}}
      ...
      <div style={{padding:"20px 22px 16px",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
          <div>
            ...
            {portfolio.description&&<div style={{fontSize:11,color:"#3a4a6a",fontFamily:"'DM Mono',monospace"}}>{portfolio.description}</div>}
          </div>
          <div style={{display:"flex",gap:6}}>
            <button onClick={onEdit} style={{background:"rgba(255,255,255,0.03)",backdropFilter:"blur(8px)",border:"1px solid rgba(255,255,255,0.08)",color:"#5a6a8a",padding:"5px 10px",borderRadius:6,cursor:"pointer",fontSize:10,fontFamily:"'DM Mono',monospace",letterSpacing:1}}
            ...
            <button onClick={onDelete} style={{background:"transparent",border:"1px solid #0e1828",color:"#5a6a8a",padding:"5px 10px",borderRadius:6,cursor:"pointer",fontSize:10,fontFamily:"'DM Mono',monospace",letterSpacing:1}}
              onMouseEnter={e=>{e.currentTarget.style.color=red;e.currentTarget.style.borderColor=red+"44";}}
              onMouseLeave={e=>{e.currentTarget.style.color="#5a6a8a";e.currentTarget.style.borderColor="#0e1828";}}>✕</button>
          </div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
          {[{label:"Invested",val:`₹${fmtCap(totalInvested)}`,color:"#c8d4f0"},{label:"Current Value",val:`₹${fmtCap(currentValue)}`,color:up?teal:red},{label:"P&L",val:`${up?"+":""}${pnlPct.toFixed(2)}%`,color:up?teal:red}].map(s=>(
      ...
      <button onClick={()=>setExpanded(v=>!v)}
        ...
        onMouseEnter={e=>e.currentTarget.style.background="rgba(255,255,255,0.02)"}
        onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
        <span style={{fontSize:9,color:"#5a6a8a",letterSpacing:3,textTransform:"uppercase",fontFamily:"'DM Mono',monospace"}}>{holdings.length} Holding{holdings.length!==1?"s":""}</span>
        <span style={{fontSize:10,color:"#3a4a6a",fontFamily:"'DM Mono',monospace"}}>{expanded?"▲ Hide":"▼ Show"}</span>
      </button>
      {expanded&&(
        <>
          {holdings.length===0?(
            <div style={{padding:"24px",textAlign:"center",color:"#3a4a6a",fontSize:11,fontFamily:"'DM Mono',monospace"}}>No holdings yet — buy shares from the Exchange tab</div>
          ):(
              ...
              <table style={{width:"100%",borderCollapse:"collapse",minWidth:520}}>
                ...
                <tbody>
                  {holdings.map(h=>{
                    ...
                    const ld=company?(liveMap[company.id]||null):null;
                    const livePrice=ld?ld.price:parseFloat(h.averagePrice);
                    const avgPrice=parseFloat(h.averagePrice),qty=parseFloat(h.quantity),invested=parseFloat(h.totalInvestment);
                    const curVal=livePrice*qty,hPnl=curVal-invested,hPnlPct=invested>0?(hPnl/invested)*100:0,hUp=hPnl>=0;
                    return <tr key={h.id} style={{borderBottom:"1px solid rgba(255,255,255,0.02)"}}
                      ...
                      <td style={{padding:"10px 14px",textAlign:"right"}}>
                        ...
                        <div style={{fontSize:9,color:hUp?teal:red,fontFamily:"'DM Mono',monospace"}}>{hUp?"+":"-"}₹{fmtCap(Math.abs(hPnl))}</div>
                      </td>
                    </tr>;
                  })}
                </tbody>
              ...
              {txLoading?(
                ...
                <div style={{maxHeight:220,overflowY:"auto"}}>
                  <table style={{width:"100%",borderCollapse:"collapse"}}>
                    ...
                    <tbody>
                      {txns.map(tx=>{
                        const isBuy=tx.transactionType==="BUY",c=isBuy?teal:red;
                        return <tr key={tx.id} style={{borderBottom:"1px solid rgba(255,255,255,0.02)"}}
                          onMouseEnter={e=>e.currentTarget.style.background="rgba(255,255,255,0.015)"}
                          onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                          ...
                          <td style={{padding:"8px 14px"}}><span style={{background:`${c}18`,border:`1px solid ${c}44`,color:c,padding:"2px 8px",borderRadius:4,fontSize:9,fontFamily:"'DM Mono',monospace",letterSpacing:1}}>{isBuy?"▲ BUY":"▼ SELL"}</span></td>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(4,8,18,0.7)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:9999,backdropFilter:"blur(24px)",WebkitBackdropFilter:"blur(24px)"}}>
      <div style={{borderRadius:18,padding:"36px 44px",textAlign:"center",minWidth:300,background:"rgba(8,15,30,0.7)",backdropFilter:"blur(24px)",WebkitBackdropFilter:"blur(24px)",border:"1px solid rgba(255,90,106,0.25)",borderTop:"2px solid #ff5a6a"}}>
        ...
        <div style={{display:"flex",gap:10,justifyContent:"center"}}>
          <button onClick={onCancel} style={{background:"transparent",border:"1px solid #0e1828",color:"#3a4a6a",padding:"9px 24px",borderRadius:8,fontFamily:"'DM Mono',monospace",fontSize:10,letterSpacing:2,cursor:"pointer"}}
            onMouseEnter={e=>{e.currentTarget.style.color="#c8d4f0";e.currentTarget.style.borderColor="#3a4a6a";}}
            onMouseLeave={e=>{e.currentTarget.style.color="#3a4a6a";e.currentTarget.style.borderColor="#0e1828";}}>CANCEL</button>
          <button onClick={onConfirm} style={{background:"rgba(255,90,106,0.12)",border:"1px solid rgba(255,90,106,0.35)",color:red,padding:"9px 24px",borderRadius:8,fontFamily:"'DM Mono',monospace",fontSize:10,letterSpacing:2,cursor:"pointer"}}
...
export default function Home({ onLogout }) {
  ...
  const { liveMap, connected } = useExchange();

  const fetchCompanies = async () => {
    try { setLoading(true); const res=await STOCK_API.get("/companies"); setCompanies(res.data); }
  ...
  const fetchOrders = async () => {
    ...
    }catch(e){console.warn("[Orders]",e.message);setOrders([]);}
    finally{setOrdersLoading(false);}
  };

  const fetchPortfolios = async () => {
    setPortfoliosLoading(true);
    try{const res=await PORTFOLIO_API.get("/portfolios");setPortfolios(Array.isArray(res.data)?res.data:[]);}
    catch(e){console.warn("[Portfolio]",e.message);setPortfolios([]);}
    finally{setPortfoliosLoading(false);}
  };

  const toggleWatchlist=(id)=>setWatchlist(prev=>{
    const n=new Set(prev);n.has(id)?n.delete(id):n.add(id);
    localStorage.setItem("sb_watchlist",JSON.stringify([...n]));return n;
  });
  const saveAlert=(a)=>setAlerts(prev=>{
    const n=[...prev.filter(x=>!(x.companyId===a.companyId&&x.direction===a.direction)),a];
    localStorage.setItem("sb_alerts",JSON.stringify(n));return n;
  });
  ...
  // Alert check on each live tick
  useEffect(()=>{
    if(!alerts.length) return;
    alerts.forEach(a=>{
      const ld=liveMap[a.companyId]; if(!ld?.price) return;
      ...
      if(Notification.permission==="granted") new Notification(`Price Alert: ${a.symbol}`,{body:`${a.symbol} hit ₹${a.targetPrice} — now ₹${ld.price.toFixed(2)}`});
  ...
  const gainers    =companies.filter(c=>getLD(c).price>0&&getLD(c).dayChangePct>=0).length;
  ...
  const filteredCompanies=companies.filter(c=>{
    ...
    const ld=getLD(c),dayPct=ld.dayChangePct;
    const matchF=filter==="All"?true:filter==="Gainers"?dayPct>=0&&ld.price>0:filter==="Losers"?dayPct<0&&ld.price>0:filter==="Circuit"?Math.abs(dayPct)>=15:true;
  ...
  return (
    <>
      ...
      {selected&&<LiveDetailModal company={selected.company} liveData={getLD(selected.company)} onClose={()=>setSelected(null)}/>}
      {portfolioModal&&<PortfolioModal mode={portfolioModal==="create"?"create":"edit"} portfolio={portfolioModal==="create"?null:portfolioModal} onClose={()=>setPortfolioModal(null)} onSaved={fetchPortfolios}/>}
      {alertModal&&<AlertModal company={alertModal} livePrice={getLD(alertModal).price} existingAlerts={alerts} onSave={saveAlert} onRemove={removeAlert} onClose={()=>setAlertModal(null)}/>}

      {/* Alert toasts */}
      <div style={{position:"fixed",bottom:24,right:24,zIndex:9999,display:"flex",flexDirection:"column",gap:10,pointerEvents:"none"}}>
        {alertToast.map(t=>{
          const a=t.alert,up=a.direction==="above",c=up?teal:red;
          return <div key={t.id} className="glass-card" style={{borderRadius:14,padding:"14px 18px",minWidth:280,borderLeft:`3px solid ${c}`,animation:"slideInRight 0.35s ease",pointerEvents:"auto"}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}><span style={{fontSize:14}}>🔔</span><span style={{fontSize:10,fontWeight:700,color:c,fontFamily:"'DM Mono',monospace",letterSpacing:1}}>PRICE ALERT</span></div>
            <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:800,fontSize:16,color:"#eef0f8"}}>{a.symbol}</div>
            <div style={{fontSize:11,color:"#6a7a9a",fontFamily:"'DM Mono',monospace",marginTop:3}}>{up?"Crossed ▲":"Dropped ▼"} ₹{fmt(a.targetPrice)} · now ₹{fmt(t.price)}</div>
          </div>;
        })}
      </div>
      ...
      <div style={{height:"100vh",display:"flex",flexDirection:"column",background:"#04080e",fontFamily:"'DM Mono',monospace",color:"#c8d4f0"}}>
        ...
        <div style={{background:"#060d1a",borderBottom:"1px solid #0e1828",height:56,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 24px",flexShrink:0,zIndex:100}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <button onClick={fetchCompanies} style={{background:"transparent",border:"1px solid #0e1828",color:"#5a6a8a",padding:"5px 12px",borderRadius:7,cursor:"pointer",fontFamily:"'DM Mono',monospace",fontSize:9,letterSpacing:2,transition:"all 0.15s"}}
              onMouseEnter={e=>{e.currentTarget.style.color=blue;e.currentTarget.style.borderColor=blue+"44";}}
              onMouseLeave={e=>{e.currentTarget.style.color="#2a3550";e.currentTarget.style.borderColor="#0e1828";}}>↻</button>
            <button onClick={()=>setShowLogout(true)} style={{background:"rgba(255,90,106,0.08)",border:"1px solid rgba(255,90,106,0.2)",color:red,padding:"5px 14px",borderRadius:7,cursor:"pointer",fontFamily:"'DM Mono',monospace",fontSize:9,letterSpacing:2,textTransform:"uppercase",transition:"all 0.15s",display:"flex",alignItems:"center",gap:6}}
        ...
        {companies.filter(c=>getLD(c).price>0).length>0&&(
          <div style={{overflow:"hidden",borderBottom:"1px solid #0e1828",background:"#060d1a",padding:"6px 0",flexShrink:0}}>
            <div style={{display:"inline-flex",gap:36,animation:"tickerScroll 35s linear infinite",whiteSpace:"nowrap"}}>
              {(()=>{const live=companies.filter(c=>getLD(c).price>0);return [...live,...live,...live].map((c,i)=>{
                const ld=getLD(c),up=ld.dayChangePct>=0,col=up?teal:red;
                return <span key={i} style={{display:"inline-flex",alignItems:"center",gap:7,fontSize:11,fontFamily:"'DM Mono',monospace"}}>
            ...
            {tab==="dashboard"&&(
              <div className="fi" style={{display:"flex",flexDirection:"column",gap:22}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
                  ...
                  <div style={{fontSize:10,color:"#4a5a7a",letterSpacing:1}}>{new Date().toLocaleDateString("en-IN",{weekday:"long",day:"numeric",month:"long",year:"numeric"})}</div>
                </div>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
                    {[["Top Gainers",topGainers,teal],["Top Losers",topLosers,red]].map(([title,list,c])=>(
                      <div key={title} className="glass-card" style={{borderRadius:14,overflow:"hidden"}}>
                        ...
                        {list.map(company=>{
                          const ld=getLD(company),up=ld.dayChangePct>=0;
                          return <div key={company.id} onClick={()=>setSelected({company,liveData:ld})}
                            ...
                            onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                            <div style={{display:"flex",alignItems:"center",gap:8}}>
                              <div style={{background:`${c}15`,border:`1px solid ${c}30`,color:c,padding:"2px 8px",borderRadius:4,fontSize:10,fontFamily:"'DM Mono',monospace",letterSpacing:1}}>{company.symbol||company.name.substring(0,5).toUpperCase()}</div>
                            ...
                            <div style={{display:"flex",alignItems:"center",gap:8}}>
                              ...
                              <span style={{fontSize:12,color:up?teal:red,fontFamily:"'DM Mono',monospace",fontWeight:600,minWidth:65,textAlign:"right"}}>{up?"+":""}{ld.dayChangePct.toFixed(2)}%</span>
                            </div>
                          </div>;
                        })}
                ...
                <div>
                  <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
                    ...
                    <span style={{fontSize:10,color:"#2a3550",fontFamily:"'DM Mono',monospace"}}>({companies.length} total · {hasLive} live)</span>
                    <button onClick={()=>setTab("exchange")} style={{marginLeft:"auto",background:"transparent",border:"1px solid #0e1828",color:"#5a6a8a",padding:"4px 12px",borderRadius:6,cursor:"pointer",fontFamily:"'DM Mono',monospace",fontSize:9,letterSpacing:2,transition:"all 0.15s"}}
                      onMouseEnter={e=>{e.currentTarget.style.color=blue;e.currentTarget.style.borderColor=blue+"44";}}
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:10}}>
                    {companies.map((c,i)=>{
                      const ld=getLD(c),live=ld.price>0,up=ld.dayChangePct>=0,color=live?(up?teal:red):"#2a3550";
                      const ac=i%4===0?blue:i%4===1?teal:i%4===2?amber:purple;
                      return <div key={c.id} onClick={()=>setSelected({company:c,liveData:ld})} className="fi"
                        ...
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
                          <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:800,fontSize:17,color:live?"#eef0f8":"#2a3550"}}>{live?`₹${fmt(ld.price)}`:"Not listed"}</div>
                          {live&&<div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:2}}>
              ...
              <div className="fi" style={{display:"flex",flexDirection:"column",gap:16}}>
                ...
                {companies.length!==hasLive&&(
                  <div style={{background:"rgba(251,191,36,0.06)",border:"1px solid rgba(251,191,36,0.2)",borderRadius:10,padding:"12px 16px",display:"flex",alignItems:"center",gap:10}}>
                    ...
                    <span style={{fontSize:11,color:amber,fontFamily:"'DM Mono',monospace"}}>{companies.length-hasLive} company/companies in stock-service (8082) not in trade-service (8083). Add with an openingPrice to enable live pricing.</span>
                  </div>
                )}
                ...
                <div className="glass-card" style={{borderRadius:14,overflow:"hidden"}}>
                  <table style={{width:"100%",borderCollapse:"collapse"}}>
                    ...
                    <tbody>
                      {filteredCompanies.map(company=>(
                        <LiveCompanyRow key={company.id} company={company} liveData={getLD(company)}
                  ...
                  {filteredCompanies.length===0&&<div style={{textAlign:"center",padding:48,color:"#4a5a7a",fontSize:11,letterSpacing:2}}>NO COMPANIES MATCH</div>}
                </div>
                <div style={{height:24}}/>
              </div>
            )}
            ...
            {tab==="orders"&&(
              <div className="fi" style={{display:"flex",flexDirection:"column",gap:16}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  ...
                  <button onClick={fetchOrders} style={{background:"transparent",border:"1px solid #0e1828",color:"#5a6a8a",padding:"7px 16px",borderRadius:7,cursor:"pointer",fontFamily:"'DM Mono',monospace",fontSize:9,letterSpacing:2,transition:"all 0.15s"}}
                    onMouseEnter={e=>{e.currentTarget.style.color=blue;e.currentTarget.style.borderColor=blue+"44";}}
                    onMouseLeave={e=>{e.currentTarget.style.color="#2a3550";e.currentTarget.style.borderColor="#0e1828";}}>↻ Refresh</button>
                </div>
                {orders.length>0&&(()=>{
                  ...
                  return <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:10}}>
                    {[{label:"Total Orders",val:orders.length,color:blue},{label:"Buy Orders",val:buys.length,color:teal},{label:"Sell Orders",val:sells.length,color:red},{label:"Executed",val:filled.length,color:teal},{label:"Traded Value",val:`₹${fmtCap(totalVal)}`,color:amber}].map(s=>(
                ...
                <div className="glass-card" style={{borderRadius:14,overflow:"hidden"}}>
                  {ordersLoading?(
                    ...
                    <div style={{textAlign:"center",padding:56}}>
                      ...
                      <div style={{fontSize:10,color:"#3a4a6a",marginTop:6}}>Please log out and log back in</div>
                    </div>
                  ):orders.length===0?(
                    <div style={{textAlign:"center",padding:56}}>
                      <div style={{fontSize:32,marginBottom:10}}>◑</div>
                    ...
                    <table style={{width:"100%",borderCollapse:"collapse"}}>
                      ...
                      <tbody>
                        {[...orders].reverse().map((order,i)=>{
                          const isBuy=order.side==="BUY",isExec=order.status==="EXECUTED"||order.status==="FILLED",isPend=order.status==="PENDING";
                          const sideCol=isBuy?teal:red,statCol=isExec?teal:isPend?amber:"#6a7a9a";
                          const company=companies.find(c=>c.id===order.companyId),val=order.price*order.quantity;
                          return <tr key={order.id} style={{borderBottom:"1px solid #090f1e",transition:"background 0.15s"}}
                            onMouseEnter={e=>e.currentTarget.style.background="rgba(0,184,255,0.02)"}
                            onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                            <td style={{padding:"13px 14px",fontSize:10,color:"#4a5a7a",fontFamily:"'DM Mono',monospace"}}>{String(orders.length-i).padStart(3,"0")}</td>
                            ...
                            <td style={{padding:"13px 14px"}}><span style={{background:`${sideCol}18`,border:`1px solid ${sideCol}44`,color:sideCol,padding:"3px 10px",borderRadius:5,fontSize:10,fontFamily:"'DM Mono',monospace",fontWeight:700,letterSpacing:1}}>{isBuy?"▲ BUY":"▼ SELL"}</span></td>
                            <td style={{padding:"13px 14px"}}><span style={{background:"rgba(167,139,250,0.1)",color:purple,padding:"3px 8px",borderRadius:4,fontSize:9,fontFamily:"'DM Mono',monospace",letterSpacing:1}}>{order.orderType||"MARKET"}</span></td>
                            ...
                            <td style={{padding:"13px 14px",textAlign:"right"}}><div style={{fontSize:13,fontWeight:700,color:"#eef0f8",fontFamily:"'Plus Jakarta Sans',sans-serif"}}>₹{fmt(order.price)}</div></td>
                            <td style={{padding:"13px 14px",textAlign:"right"}}><div style={{fontSize:12,color:blue,fontFamily:"'DM Mono',monospace",fontWeight:600}}>₹{fmt(val)}</div></td>
                            <td style={{padding:"13px 14px"}}>
                              <div style={{display:"flex",alignItems:"center",gap:5}}>
                            ...
                            <td style={{padding:"13px 14px",fontSize:10,color:"#3a4a6a",fontFamily:"'DM Mono',monospace"}}>{fmtTime(order.createdAt)}</td>
                          </tr>;
                        })}
                      </tbody>
                    </table>
                ...
                {orders.length>0&&<div style={{textAlign:"center",fontSize:10,color:"#7a8aaa",letterSpacing:2}}>{orders.length} ORDERS · USER #{localStorage.getItem("userId")||"?"}</div>}
                <div style={{height:24}}/>
              </div>
            )}
            ...
            {tab==="portfolio"&&(
              <div className="fi" style={{display:"flex",flexDirection:"column",gap:16}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
                  ...
                  <button onClick={()=>setPortfolioModal("create")} style={{display:"flex",alignItems:"center",gap:8,background:"rgba(0,212,160,0.12)",border:"1px solid rgba(0,212,160,0.35)",color:teal,padding:"9px 18px",borderRadius:9,cursor:"pointer",fontFamily:"'DM Mono',monospace",fontSize:10,letterSpacing:2,textTransform:"uppercase",transition:"all 0.15s"}}
                    onMouseEnter={e=>e.currentTarget.style.background="rgba(0,212,160,0.22)"}
                    onMouseLeave={e=>e.currentTarget.style.background="rgba(0,212,160,0.12)"}>+ New Portfolio</button>
                </div>
                {portfoliosLoading?(
                  ...
                  <div style={{background:"#0a1020",border:"1px solid #0e1828",borderRadius:16,padding:"64px 28px",textAlign:"center"}}>
                    ...
                    <button onClick={()=>setPortfolioModal("create")} style={{background:"rgba(0,212,160,0.12)",border:"1px solid rgba(0,212,160,0.35)",color:teal,padding:"10px 24px",borderRadius:8,cursor:"pointer",fontFamily:"'DM Mono',monospace",fontSize:10,letterSpacing:2}}>+ Create First Portfolio</button>
                  </div>
                ):(
                  <>
                    {(()=>{
                      const totalInvested=portfolios.reduce((sum,p)=>sum+(p.holdings||[]).reduce((s,h)=>s+parseFloat(h.totalInvestment||0),0),0);
                      ...
                      return <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
                        {[{icon:"◧",label:"Portfolios",val:portfolios.length,color:blue},{icon:"⬡",label:"Active Holdings",val:totalHoldings,color:teal},{icon:"◈",label:"Unique Stocks",val:totalStocks,color:purple},{icon:"₹",label:"Total Invested",val:`₹${fmtCap(totalInvested)}`,color:amber}].map(s=>(
                    ...
                    <div style={{display:"flex",gap:0,background:"#060d1a",borderRadius:10,padding:3,border:"1px solid #0e1828"}}>
                      {[{id:"holdings",label:"Holdings"},{id:"sector",label:"Sector Breakdown"},{id:"sip",label:"SIP Simulator"}].map(t=>(
                    ...
                    {portfolioSubTab==="holdings"&&(
                      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(420px,1fr))",gap:14}}>
                        {portfolios.map(p=>(
                          <PortfolioCard key={p.id} portfolio={p} liveMap={liveMap} companies={companies}
                            onEdit={()=>setPortfolioModal(p)}
                            onDelete={async()=>{
                              if(!window.confirm(`Delete "${p.name}"?`)) return;
                              try{await PORTFOLIO_API.delete(`/portfolios/${p.id}`);fetchPortfolios();}
                    ...
                    {portfolioSubTab==="sector"&&<SectorDonut portfolios={portfolios} liveMap={liveMap} companies={companies}/>}
                    {portfolioSubTab==="sip"&&<SipSimulator companies={companies} liveMap={liveMap}/>}
                  </>
                )}
                <div style={{height:24}}/>
              </div>
              ...
              <div className="fi" style={{display:"flex",flexDirection:"column",gap:16}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    ...
                    <button onClick={()=>setTab("exchange")} style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.08)",color:"#5a6a8a",padding:"7px 14px",borderRadius:8,cursor:"pointer",fontFamily:"'DM Mono',monospace",fontSize:9,letterSpacing:2,transition:"all 0.15s"}}
                ...
                {watchlist.size===0?(
                  <div className="glass-card" style={{borderRadius:18,padding:"64px 28px",textAlign:"center"}}>
                    ...
                    <button onClick={()=>setTab("exchange")} style={{background:"rgba(251,191,36,0.1)",border:"1px solid rgba(251,191,36,0.3)",color:amber,padding:"10px 24px",borderRadius:8,cursor:"pointer",fontFamily:"'DM Mono',monospace",fontSize:10,letterSpacing:2}}>GO TO EXCHANGE</button>
                  </div>
                ):(
                  <>
                    {(()=>{
                      ...
                      return <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}>
                        {[{label:"Watching",val:watchlist.size,color:amber},{label:"Up Today",val:gW,color:teal},{label:"With Alerts",val:aW,color:blue}].map(s=>(
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(270px,1fr))",gap:12}}>
                      {companies.filter(c=>watchlist.has(c.id)).map(c=>(
                        <WatchlistCard key={c.id} company={c} liveData={getLD(c)} alerts={alerts}
                          onRemove={toggleWatchlist} onSetAlert={co=>setAlertModal(co)}
                        ...
                        <table style={{width:"100%",borderCollapse:"collapse"}}>
                          ...
                          <tbody>
                            {alerts.map((a,i)=>{
                              ...
                              const ld=comp?getLD(comp):null,cur=ld?ld.price:0;
                              const gap=cur>0?((a.targetPrice-cur)/cur*100):null,dirUp=a.direction==="above";
                              ...
                              return <tr key={i} style={{borderBottom:"1px solid rgba(255,255,255,0.03)"}}
                                ...
                                <td style={{padding:"11px 16px",textAlign:"right"}}>
                                  <span style={{background:dirUp?"rgba(0,212,160,0.1)":"rgba(255,90,106,0.1)",border:`1px solid ${dirUp?teal:red}33`,color:dirUp?teal:red,fontSize:9,padding:"2px 8px",borderRadius:4,fontFamily:"'DM Mono',monospace"}}>{dirUp?"▲ ABOVE":"▼ BELOW"}</span>
                                </td>
                                ...
                                <td style={{padding:"11px 16px",textAlign:"right"}}>
                                  {hit?<span style={{background:"rgba(0,212,160,0.1)",border:"1px solid rgba(0,212,160,0.3)",color:teal,fontSize:9,padding:"2px 8px",borderRadius:4,fontFamily:"'DM Mono',monospace"}}>✓ HIT</span>:gap!==null?<span style={{fontSize:11,color:amber,fontFamily:"'DM Mono',monospace"}}>{gap>=0?"+":""}{gap.toFixed(2)}%</span>:"—"}
                                </td>
                                <td style={{padding:"11px 16px",textAlign:"right"}}>
                                  <button onClick={()=>removeAlert(a.companyId,a.direction)} style={{background:"rgba(255,90,106,0.08)",border:"1px solid rgba(255,90,106,0.2)",color:red,padding:"4px 10px",borderRadius:5,cursor:"pointer",fontSize:9,fontFamily:"'DM Mono',monospace"}}>Remove</button>
                                </td>
                              </tr>;
                            })}
                          </tbody>
